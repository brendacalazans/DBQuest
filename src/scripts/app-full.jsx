(function() {
    ...
    const Edit2 = memo(() => <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M17 3a2.85 2.85 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5 Z"/><path d="m15 5 4 4"/></svg>); // Novo ícone
...
// 1. COLE ESTE NOVO MÓDULO "SQL INTERMEDIÁRIO" AQUI
    ...
    function App() {
        ...
        // Efeito: Carregar Dados do Usuário e Trilha (CORRIGIDO)
        useEffect(() => {
            ...
            // Observador de Dados do Usuário (LÓGICA DE VIDA CORRIGIDA AQUI)
            let offUserProgress = () => {};
            if (userId) {
                ...
                offUserProgress = onValue(userRef, async (snapshot) => {
                    if (snapshot.exists()) {
                        ...
                        const lastResetDateNum = gamification.lastLifeResetDate || 0;
                        // Converte para objeto Date para comparar dias
                        const lastResetDate = new Date(lastResetDateNum);
                        lastResetDate.setHours(0,0,0,0);

                        let lives = gamification.lives;
                        let cooldown = data.cooldownUntil ? new Date(data.cooldownUntil).getTime() : 0;
                        
                        const updates = {};
                        let needsUpdate = false;

                        // 1. LÓGICA CORRIGIDA: Só reseta se a data salva for ANTERIOR a hoje
                        if (lastResetDate.getTime() < todayTimestamp) {
                            // É um novo dia!
                            if (lives < 5) {
                                updates['gamification/lives'] = 5;
                                updates['cooldownUntil'] = null;
                                updates['gamification/lastLifeResetDate'] = todayTimestamp; // Atualiza para hoje
                                lives = 5;
                                cooldown = null;
                                needsUpdate = true;
                                console.log("Novo dia detectado: Vidas resetadas.");
                            } else {
                                // Já tem 5 vidas, mas precisamos atualizar a data para não checar de novo hoje
                                updates['gamification/lastLifeResetDate'] = todayTimestamp;
                                needsUpdate = true;
                            }
                        }

                        // 2. Check se o cooldown de 24h acabou (caso extremo)
                        ...
                        // 3. Check de Streak
                        const lastCompleted = gamification.lastCompletedLessonDate 
                            ? new Date(gamification.lastCompletedLessonDate) 
                            : null;
                        ...
                        if (lastCompleted) {
                            ...
                            // Se a última lição foi antes de ontem, zera o streak
                            if (lastCompleted.getTime() < yesterday.getTime() && gamification.streak > 0) {
                        ...
                        if (needsUpdate) {
                             ...
                             // Não atualizamos o estado local aqui. 
                             // O Firebase vai disparar este evento de novo com os dados atualizados.
                        } else {
                            // Se NÃO precisa de update, sincroniza o estado local com o banco
                            setUserProgress({
                                ...gamification,
                                username: data.name || auth.currentUser.displayName || 'Aluno',
                                avatar: data.avatar || '👤',
                                cooldownUntil: data.cooldownUntil,
                                lives: lives // Garante que usa a vida que veio do banco
                            });
                        }
                        
                        // Garante ranking atualizado
                        ...
                        // Cria usuário novo (Código padrão mantido)
                        const newUser = {
                        ...
                        await set(userRef, newUser);
                        // Não precisa setUserProgress aqui, o onValue vai rodar de novo
...
        };
            ...
            return (
                <main className="max-w-5xl mx-auto px-4 md:px-6 py-8 animate-fade-in">
                    ...
                    <div className="bg-black/20 backdrop-blur-md rounded-2xl p-4 md:p-8 border border-white/10">
                        ...
                        {/* --- FILTROS CORRIGIDOS --- */}
                        {/* Adicionei 'flex-nowrap' no container e 'flex-shrink-0' nos botões para não esmagarem */}
                        <div className="flex items-center gap-2 mb-6 overflow-x-auto pb-2 -mx-4 px-4 md:mx-0 md:px-0 flex-nowrap">
                            <button onClick={() => onFilterChange('all')} className={`flex-shrink-0 px-4 py-2 rounded-lg text-sm font-medium transition-colors whitespace-nowrap ${filterType === 'all' ? 'bg-white/10 text-white' : 'bg-white/5 text-white/60 hover:bg-white/10 hover:text-white'}`}>Todos</button>
                            <button onClick={() => onFilterChange('article')} className={`flex-shrink-0 px-4 py-2 rounded-lg text-sm font-medium transition-colors whitespace-nowrap flex items-center gap-2 ${filterType === 'article' ? 'bg-white/10 text-white' : 'bg-white/5 text-white/60 hover:bg-white/10 hover:text-white'}`}><FileText className="w-4 h-4" />Artigos</button>
                            <button onClick={() => onFilterChange('lesson')} className={`flex-shrink-0 px-4 py-2 rounded-lg text-sm font-medium transition-colors whitespace-nowrap flex items-center gap-2 ${filterType === 'lesson' ? 'bg-white/10 text-white' : 'bg-white/5 text-white/60 hover:bg-white/10 hover:text-white'}`}><GraduationCap className="w-4 h-4" />Aulas</button>
                            <button onClick={() => onFilterChange('theory')} className={`flex-shrink-0 px-4 py-2 rounded-lg text-sm font-medium transition-colors whitespace-nowrap flex items-center gap-2 ${filterType === 'theory' ? 'bg-white/10 text-white' : 'bg-white/5 text-white/60 hover:bg-white/10 hover:text-white'}`}><BookOpen className="w-4 h-4" />Teoria</button>
                            <button onClick={() => onFilterChange('practice')} className={`flex-shrink-0 px-4 py-2 rounded-lg text-sm font-medium transition-colors whitespace-nowrap flex items-center gap-2 ${filterType === 'practice' ? 'bg-white/10 text-white' : 'bg-white/5 text-white/60 hover:bg-white/10 hover:text-white'}`}><PenTool className="w-4 h-4" />Prática</button>
                        </div>
                                        ...
                                        <div className="flex items-start gap-4 md:gap-5">
                                            ...
                                            <div className="flex-1 min-w-0">
                                                ...
                                                <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2 mb-2">
                                                    ...
                                                    {/* --- LABEL COM LARGURA FIXA NO DESKTOP --- */}
                                                    {/* 'md:w-24 justify-center': Fixa largura no PC e centraliza o conteúdo */}
                                                    {/* 'self-start': Mantém tamanho natural no mobile */}
                                                    <div className={`self-start px-3 py-1 rounded-lg text-xs font-bold border flex items-center md:justify-center gap-1 whitespace-nowrap flex-shrink-0 md:w-24 ${contentType.color}`}>
    ...
    const ArticleView = memo(({ currentLesson, onComplete, onBack }) => {
       ...
       const callGeminiAPI = useCallback(async (payload, retries = 3, delay = 1000) => {
            const apiKey = "AIzaSyCM2J5xASXRUmh3CGjgxO3xCOrbe8zN1Fc";
